...
unittest {
    ...
    assert(isClose(kde(1), normalPDF(1), 1e-2, 1e-2));
    assert(isClose(kde.cdf(1), normalCDF(1), 1e-2, 1e-2));
    assert(isClose(kde.cdfr(1), normalCDFR(1), 1e-2, 1e-2));
...
unittest {
    // Values from R.
    assert(isClose(scottBandwidth([1,2,3,4,5]), 1.14666, 1e-2, 1e-2));
    assert(isClose(scottBandwidth([1,2,2,2,2,8,8,8,8]), 2.242446, 1e-2, 1e-2));
}
